...
- 6.x återhärdad 2026-03-29: login-start har nu pending-session-rate-limit och unresolved-identifier-lockout, TOTP och passkey har faktorbaserad invalid-attempt-lockout med session-revokering, och TOTP-hemligheter är flyttade från rå `authFactors` till sealed durable secret envelopes; verifierat i nya phase 6 unit/API-sviter och runbook `docs/runbooks/auth-rate-limit-and-lockout.md`.
