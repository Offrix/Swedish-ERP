...
test("Phase 18.2 API exposes pilot cohorts with segment acceptance and evidence", async () => {
  const platform = createApiPlatform({
    clock: () => new Date("2026-03-29T19:30:00Z")
  });
  ...
  try {
    ...
    const financeReady = await bootstrapFinanceReadyCompany({
      baseUrl,
      adminEmail: "phase18-cohort-api-owner@example.test",
      legalName: "Phase 18 Cohort API AB",
      orgNumber: "559900-9494"
    });
    ...
    const financeApprover = platform.createCompanyUser({
      ...
      companyId: financeReady.companyId,
      email: "phase18-cohort-api-finance@example.test",
      displayName: "Phase 18 Cohort API Finance",
      roleCode: "approver",
      requiresMfa: true
    });
    const supportApprover = platform.createCompanyUser({
      sessionToken: companyToken,
      companyId: financeReady.companyId,
      email: "phase18-cohort-api-support@example.test",
      displayName: "Phase 18 Cohort API Support",
    ...
    const pilot = await requestJson(baseUrl, "/v1/pilot/executions", {
      ...
      body: {
        companyId: financeReady.companyId,
        label: "API service project pilot",
    ...
    const cohort = await requestJson(baseUrl, "/v1/pilot/cohorts", {
      ...
      body: {
        ...
        label: "API service project cohort"
      }
    });
    assert.equal(cohort.status, "planned");

    const attached = await requestJson(baseUrl, `/v1/pilot/cohorts/${cohort.pilotCohortId}/pilots`, {
      method: "POST",
      token: companyToken,
      body: {
    ...
    assert.equal(attached.coverageSummary.readyForAcceptance, true);
    ...
    const listed = await requestJson(baseUrl, `/v1/pilot/cohorts?companyId=${financeReady.companyId}&segmentCode=service_project_company`, {
      token: companyToken
    });
    assert.equal(listed.items.some((item) => item.pilotCohortId === cohort.pilotCohortId && item.status === "accepted"), true);

    const evidence = await requestJson(baseUrl, `/v1/pilot/cohorts/${cohort.pilotCohortId}/evidence`, {
      token: companyToken
    });
    assert.equal(evidence.evidenceBundle.bundleType, "pilot_cohort");
  } finally {
...
async function bootstrapFinanceReadyCompany({ baseUrl, adminEmail, legalName, orgNumber }) {
  const onboardingRun = await requestJson(baseUrl, "/v1/tenant/bootstrap", {
    ...
    body: {
      ...
      orgNumber,
      adminEmail,
      adminDisplayName: "Phase 18 Cohort API Owner"
    }
  });
