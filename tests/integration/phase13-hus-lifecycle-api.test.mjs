...
test("Phase 13.1 API hardens HUS lifecycle gates, direct transport and deadline semantics", async () => {
  ...
  try {
    ...
    const husCase = await requestJson(baseUrl, "/v1/hus/cases", {
      ...
      body: {
        companyId: COMPANY_ID,
        caseReference: "HUS-13-API-001",
        serviceTypeCode: "rot",
        workCompletedOn: "2026-12-10",
        housingFormCode: "smallhouse",
        propertyDesignation: "UPPSALA SUNNERSTA 1:30",
    ...
    await requestJson(baseUrl, `/v1/hus/cases/${husCase.husCaseId}/invoice`, {
      ...
      body: {
        companyId: COMPANY_ID,
        invoiceNumber: "HUS-13-API-INV-001",
    ...
    await requestJson(baseUrl, `/v1/hus/cases/${husCase.husCaseId}/payments`, {
      ...
      body: {
        ...
        paymentReference: "BG-HUS-13-API-001"
      }
    });
    ...
    assert.equal(readiness.buyerCapacities.length, 1);
    ...
    assert.equal(claim.transportProfile.deliveryChannelCode, "direct_api");
    assert.equal(claim.buyerAllocations.length, 1);

    const lockedReclassify = await requestJson(baseUrl, `/v1/hus/cases/${husCase.husCaseId}/classify`, {
      method: "POST",
      token: sessionToken,
      expectedStatus: 409,
      body: {
    ...
    assert.equal(lockedReclassify.errorCode, "hus_case_claim_fields_locked");

    const prematureDecision = await requestJson(baseUrl, `/v1/hus/claims/${claim.husClaimId}/decisions`, {
      method: "POST",
      token: sessionToken,
      expectedStatus: 409,
      body: {
        companyId: COMPANY_ID,
        decisionDate: "2026-12-16",
        approvedAmount: 3000,
        reasonCode: "accepted"
      }
    });
    assert.equal(prematureDecision.errorCode, "hus_claim_not_decidable");
    ...
    const blockedPayout = await requestJson(baseUrl, `/v1/hus/claims/${claim.husClaimId}/payouts`, {
      method: "POST",
      token: sessionToken,
      expectedStatus: 409,
      body: {
        companyId: COMPANY_ID,
        payoutDate: "2026-12-20",
        payoutAmount: 2500
      }
    });
    assert.equal(blockedPayout.errorCode, "hus_claim_difference_unresolved");
