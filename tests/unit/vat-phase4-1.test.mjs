...
test("Phase 4.1 returns traceable VAT decisions and replays historical rule packs deterministically", () => {
  ...
  assert.equal(historical.vatDecision.rulePackId, "vat-se-2025.4");
  assert.equal(historical.vatDecision.status, "decided");
  assert.ok(historical.vatDecision.inputsHash);
  assert.match(historical.vatDecision.explanation.join(" "), /rule_pack_id=vat-se-2025\.4/);
  ...
  assert.equal(current.vatDecision.rulePackId, "vat-se-2026.1");
