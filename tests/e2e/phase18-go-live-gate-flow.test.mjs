...
test("Phase 18.6 end-to-end reaches approved general availability gate after pilots, parity, advantage and UI freeze", async () => {
  const platform = createApiPlatform({
    clock: () => new Date("2026-03-30T00:45:00Z")
  });
  ...
  try {
    ...
    const financeReady = await bootstrapFinanceReadyCompany({
      baseUrl,
      adminEmail: "phase18-go-live-e2e-owner@example.test",
      legalName: "Phase 18 GA E2E AB",
    ...
    const financeApprover = platform.createCompanyUser({
      ...
      companyId: financeReady.companyId,
      email: "phase18-go-live-e2e-finance@example.test",
      displayName: "Phase 18 Go Live E2E Finance",
      roleCode: "approver",
      requiresMfa: true
    });
    const supportApprover = platform.createCompanyUser({
      sessionToken: companyToken,
      companyId: financeReady.companyId,
      email: "phase18-go-live-e2e-support@example.test",
      displayName: "Phase 18 Go Live E2E Support",
    ...
    const firstGate = await requestJson(baseUrl, "/v1/release/go-live-gates", {
      method: "POST",
      token: companyToken,
      expectedStatus: 201,
      body: {
    ...
    const secondGate = await requestJson(baseUrl, "/v1/release/go-live-gates", {
      method: "POST",
      token: companyToken,
      expectedStatus: 201,
      body: {
    ...
    assert.equal(firstGate.status, "approved");
    assert.equal(secondGate.status, "approved");
    assert.deepEqual(firstGate.summary.missingPilotSegmentCodes, []);
    assert.deepEqual(firstGate.summary.missingParityCategories, []);
    assert.equal(firstGate.summary.greenChecklistCodes.length, FINAL_CHECKLIST_CODES.length);
    assert.equal(secondGate.summary.greenChecklistCodes.length, FINAL_CHECKLIST_CODES.length);
  } finally {
...
async function bootstrapFinanceReadyCompany({ baseUrl, adminEmail, legalName, orgNumber }) {
  const onboardingRun = await requestJson(baseUrl, "/v1/tenant/bootstrap", {
    ...
    body: {
      ...
      orgNumber,
      adminEmail,
      adminDisplayName: "Phase 18 Go Live E2E Owner"
    }
  });
