...
test("Phase 12.6 e2e payroll flow creates and settles Kronofogden remittance", async () => {
  ...
  try {
    ...
    await requestJson(baseUrl, "/v1/payroll/garnishments", {
      method: "POST",
      token: sessionToken,
      expectedStatus: 201,
      body: {
        ...
        authorityCaseReference: "KFM-E2E-2026-0001",
    ...
    assert.equal(remittance.status, "payment_order_ready");

    const settled = await requestJson(
      baseUrl,
      `/v1/payroll/garnishment-remittances/${remittance.remittanceInstructionId}/settle`,
      {
        method: "POST",
        token: sessionToken,
        body: {
          companyId: COMPANY_ID,
          paymentOrderReference: "E2E-KFM-SETTLE-202603"
        }
      }
    );
...
async function createMonthlyEmployee({ baseUrl, token, givenName, familyName, identityValue }) {
  ...
  const employment = await requestJson(baseUrl, `/v1/hr/employees/${employee.employeeId}/employments`, {
    ...
    body: {
      ...
      jobTitle: "Garnishment e2e tester",
